    ...
    for basin in basins:
        ...
        for card in cards:
            ...
            if(find(card,'WWW999') and find(card,"%s.%s"%(basin,year)) ):
                tt=card.split()
                tt=tt[3].split('/')
                stmNN=tt[0].upper()[0:3]
                stm9X=tt[1].upper()[0:3]
                print stmNN,stm9X
                pNNs=glob.glob("%s/%s/*/%s*/%s-sum.txt"%(sbtSrcDir,year,stmNN,stmNN))
                pNN=pNNs[0]
                pNNBTs=glob.glob("%s/%s/*/%s*/%s-sum-BT.txt"%(sbtSrcDir,year,stmNN,stmNN))
                pNNBT=pNNBTs[0]
                p9Xs=glob.glob("%s/%s/*/%s*/%s-sum.txt"%(sbtSrcDir,year,stm9X,stm9X))
                p9X=p9Xs[0]
                
                print pNN,p9X,pNNBT
                cmd="meld %s %s"%(pNN,p9X)
                runcmd(cmd)
                
                cmd="meld %s %s"%(pNN,pNNBT)
                runcmd(cmd)
